...
import { getAlertStatus } from "../Component/AlertComponent/handleAlertApi";
...
const MainContent = (props) => {
  ...
  useEffect(() => {
    if (serviceAlertData.serverId) {
      ...
      getAlertStatus(token, serviceAlertData.serverId, setAlertList, alertList);
    } else {
      setServerSelected(false);
    } //eslint-disable-next-line
  }, [serviceAlertData.serverId, token]);
  return (
    <div className="col-xxl-10 containt">
      ...
      <div className="External-link Jenkins">
        <a
          href={`${process.env.REACT_APP_Jenkins_Base_URL}/${service_name}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          Jenkins
      ...
      <div className="External-link Sample-Jobs">
        <a
          href={`${process.env.REACT_APP_Sample_Jobs_URL}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          Sample jobs
